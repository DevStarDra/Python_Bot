...
def connect_room_cb(username_room,proxy):
    ...
    def on_open(ws):
        def apisend():
            url = requests.get(
                'http://localhost:5000/api?action=newbot&model_username={}&ping={}&botid={}'.format(username_room, str(
                    int(time.time())), bid))
            while True:
                'check by server'
                allinfo = json.loads(requests.get('http://localhost:5000/api?action=get_models&web=ctb').text)
